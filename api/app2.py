...
@app.route('/')
def home():
    return jsonify({"message": "Selamat Datang Di Sistem Antrian Kategori"})
...
def consumer_daemon():
    def callback(ch, method, properties, body):
        ...
        try:
            categories_data = json.loads(body)
            # Log the product data to know what is received
            print("Received category data:", categories_data)
            
            # Panggil fungsi yang sesuai berdasarkan tipe operasi
            operation = categories_data.get('operation')
            if operation == 'add':
                send_to_external_api(categories_data, 'POST')
            elif operation == 'update':
                send_to_external_api(categories_data, 'PUT')
            elif operation == 'delete':
                send_to_external_api(categories_data, 'DELETE')
            else:
...
def send_to_external_api(categories_data, method):
    """Mengirimkan data ke API eksternal sesuai tipe operasi."""
    urladd = 'http://localhost/onlinestorev2/index.php/api/categories'
    headers = {
        'Authorization': f'Bearer {categories_data["token"]}',
    ...
    while retries < max_retries:
        try:
            if method == 'POST':
                categories_data_infoadd = {
                    'name': categories_data['categories'][0]['name'],
                    'description': categories_data['categories'][0]['description'],
                }
                response = requests.post(urladd, headers=headers, json=categories_data_infoadd)
            elif method == 'PUT':
                categories_data_infoupdate = {
                    'id': categories_data['categories'][0]['id'],
                    'name': categories_data['categories'][0]['name'],
                    'description': categories_data['categories'][0]['description'],
                }
                urlupdatedelete = f'http://localhost/onlinestorev2/index.php/api/categories/{categories_data_infoupdate["id"]}'
                response = requests.put(urlupdatedelete, headers=headers, json=categories_data_infoupdate)
            elif method == 'DELETE':
                categories_data_infodelete = {'id': categories_data['categories'][0]['id']}  # Ambil ID dari 'category'
                urlupdatedelete = f'http://localhost/onlinestorev2/index.php/api/categories/{categories_data_infodelete["id"]}'
                response = requests.delete(urlupdatedelete, headers=headers, json=categories_data_infodelete)
                print(f"Token: {categories_data['token']}")
